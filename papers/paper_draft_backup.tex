...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\small MLP Coursework 4 -- Final Report (s2286795)}
\fancyfoot[C]{\thepage}
...
% Placeholder command for numbers to fill in
\newcommand{\TODO}[1]{\textcolor{red}{[#1]}}

\bibliographystyle{abbrvnat}
\setcitestyle{authoryear,open={(},close={)}}

\title{\vspace{-1cm}Comparing Multimodal Fusion Strategies for Fine-Grained\\Grocery Product Identification}
\author{s2286795}
\date{}
...
\end{equation}
where $[\cdot;\cdot]$ denotes concatenation and $W_c \in \mathbb{R}^{256 \times 512}$. This imposes no inductive bias about modality importance; the downstream classifier must learn how to weight the features. Its simplicity makes it a useful baseline, but it cannot dynamically adjust when one modality is uninformative. The full architecture is illustrated in Figure~\ref{fig:concat} (Appendix~\ref{sec:appendix}).
...
When the text modality is uninformative (e.g., a \texttt{[UNK]} placeholder for unpackaged fruit), the gate can learn values close to 1, effectively reducing the model to image-only classification. The full architecture is illustrated in Figure~\ref{fig:gated} (Appendix~\ref{sec:appendix}).
...
\end{equation}
where $f_{\text{out}}$ is a linear projection with ReLU, dropout, and layer normalisation. While cross-attention is the most expressive, it is also the most computationally expensive and potentially data-hungry, which may limit its effectiveness on our small dataset. The full architecture is illustrated in Figure~\ref{fig:crossattn} (Appendix~\ref{sec:appendix}).
...
This approach has two practical advantages: (1) no text encoder at inference time, reducing model size and latency; (2) interpretable per-class scores. Its disadvantage is requiring predefined class names, limiting scalability. The full architecture is illustrated in Figure~\ref{fig:scorefusion} (Appendix~\ref{sec:appendix}).
...
With only ${\sim}32$ images per class, overfitting is the primary challenge. We employ: (1) staged pretraining of encoders before end-to-end training; (2) 8 data augmentation strategies (Table~\ref{tab:augmentation}); (3) heavy regularisation (dropout $p = 0.3$--$0.5$, label smoothing $\epsilon = 0.1$, weight decay $10^{-4}$, gradient clipping); (4) text encoder layer freezing; (5) compact models (TinyBERT 14.5M params, MobileNetV3); (6) early stopping (patience 15 epochs); and (7) differential learning rates ($0.1\times$ for pretrained encoders). As shown in Table~\ref{tab:overfitting}, all models exhibit train-val gaps in the 14--16\% range, indicating controlled overfitting.
...
Table~\ref{tab:params} summarises the parameter counts for all models.
...
Table~\ref{tab:main_results} summarises the performance of all five models on the held-out test set.
...
Figure~\ref{fig:training_curves} shows the validation accuracy curves. All models converge within 50 epochs, with score fusion achieving the highest accuracy early and maintaining its lead. The top-5 curves (right) are tightly grouped above 90\%.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{trainig_curve.png}
    \caption{Validation accuracy during Stage~2 training. \textit{Left:} Top-1 accuracy. \textit{Right:} Top-5 accuracy. Score fusion achieves the highest accuracy and converges smoothly, while the neural fusion methods show more fluctuation due to their larger parameter counts.}
    \label{fig:training_curves}
\end{figure}

\subsection{Modality Contribution Analysis}

Since only 37\% of images have OCR text, we evaluate all models on the subset of test images with non-empty OCR output (Table~\ref{tab:text_subset}). This isolates the effect of text fusion on products where text is actually available.
...
Table~\ref{tab:freezing} shows the effect of unfreezing different numbers of TinyBERT transformer layers on weighted F1 across all three neural fusion strategies. TinyBERT has 4 transformer layers; ``0 unfrozen'' means all layers are frozen (text encoder used as a fixed feature extractor), while ``2 unfrozen'' means the top 2 layers are fine-tuned on the OCR data.
...
\bibitem[Brady et~al.(2013)]{brady2013visual}
Brady, E., Morris, M.~R., Zhong, Y., White, S., \& Bigham, J.~P. (2013).
\newblock Visual challenges in the everyday lives of blind people.
\newblock \emph{Proceedings of CHI} (pp.\ 2117--2126). ACM.

\bibitem[Devlin et~al.(2019)]{devlin2019bert}
Devlin, J., Chang, M., Lee, K., \& Toutanova, K. (2019).
\newblock BERT: Pre-training of deep bidirectional transformers for language understanding.
\newblock \emph{NAACL-HLT 2019} (pp.\ 4171--4186).

\bibitem[Garg et~al.(2025)]{garg2025trained}
Garg, K., Tang, X., Heo, J., et~al. (2025).
\newblock ``It's trained by non-disabled people'': Evaluating how image quality affects product captioning with VLMs.
\newblock \emph{arXiv:2511.08917}.

\bibitem[George et~al.(2015)]{george2015fine}
George, M., Mircic, D., S\"{o}r\"{o}s, G., Floerkemeier, C., \& Mattern, F. (2015).
\newblock Fine-grained product class recognition for assisted shopping.
\newblock \emph{ICCV Workshops} (pp.\ 154--161).

\bibitem[Georgiadis et~al.(2021)]{georgiadis2021products6k}
Georgiadis, K., Kordopatis-Zilos, G., et~al. (2021).
\newblock Products-6K: A large-scale groceries product recognition dataset.
\newblock \emph{PETRA '21}. ACM.

\bibitem[Jiao et~al.(2020)]{jiao2020tinybert}
Jiao, X., et~al. (2020).
\newblock TinyBERT: Distilling BERT for natural language understanding.
\newblock \emph{Findings of EMNLP 2020} (pp.\ 4163--4174).

\bibitem[Howard et~al.(2019)]{howard2019mobilenet}
Howard, A., et~al. (2019).
\newblock Searching for MobileNetV3.
\newblock \emph{ICCV 2019} (pp.\ 1314--1324).

\bibitem[Klasson et~al.(2019)]{klasson2019hierarchical}
Klasson, M., Zhang, C., \& Kjellstr\"{o}m, H. (2019).
\newblock A hierarchical grocery store image dataset with visual and semantic labels.
\newblock \emph{IEEE WACV} (pp.\ 1--9).

\bibitem[Klasson et~al.(2020)]{klasson2020using}
Klasson, M., Zhang, C., \& Kjellstr\"{o}m, H. (2020).
\newblock Using variational multi-view learning for classification of grocery items.
\newblock \emph{Patterns}, 1(8), 100143.

\bibitem[Krizhevsky et~al.(2012)]{krizhevsky2012imagenet}
Krizhevsky, A., Sutskever, I., \& Hinton, G.~E. (2012).
\newblock ImageNet classification with deep convolutional neural networks.
\newblock \emph{NeurIPS 2012} (pp.\ 1097--1105).

\bibitem[Mathis \& Sch\"{o}ning(2025)]{mathis2025lifeinsight}
Mathis, F. \& Sch\"{o}ning, J. (2025).
\newblock Lifeinsight: Design and evaluation of an AI-powered assistive wearable.
\newblock \emph{Proceedings of CHI 2025}. ACM.

\bibitem[Pettersson et~al.(2024)]{pettersson2024multimodal}
Pettersson, T., Riveiro, M., \& L\"{o}fstr\"{o}m, T. (2024).
\newblock Multimodal fine-grained grocery product recognition using image and OCR text.
\newblock \emph{Machine Vision and Applications}, 35, 79.

\bibitem[Selvam et~al.(2024)]{selvam2024bnfrnn}
Selvam, P., Faheem, M., et~al. (2024).
\newblock Batch normalization free rigorous feature flow neural network for grocery product recognition.
\newblock \emph{IEEE Access}, 12, 68364--68377.

\bibitem[Tan \& Le(2019)]{tan2019efficientnet}
Tan, M. \& Le, Q. (2019).
\newblock EfficientNet: Rethinking model scaling for convolutional neural networks.
\newblock \emph{ICML 2019} (pp.\ 6105--6114).

\bibitem[Vaswani et~al.(2017)]{vaswani2017attention}
Vaswani, A., et~al. (2017).
\newblock Attention is all you need.
\newblock \emph{NeurIPS 2017} (pp.\ 5998--6008).
...
Figures~\ref{fig:concat}--\ref{fig:scorefusion} provide detailed architecture diagrams for the four fusion strategies evaluated in this work. All three neural fusion methods (Figures~\ref{fig:concat}--\ref{fig:crossattn}) share the same MobileNetV3 image encoder and TinyBERT text encoder; only the fusion module differs. Score fusion (Figure~\ref{fig:scorefusion}) replaces the neural text encoder with a fuzzy string matcher.

\begin{figure*}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{concat.jpg}
...
\begin{figure*}[h]
    \centering
    \includegraphics[width=\textwidth]{OCR_score_macthing.jpg}
